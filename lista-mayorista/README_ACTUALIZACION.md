...
- `Lista de Precio MAYORISTA - Base de Datos.xlsx`: única fuente externa para actualizar precios automáticamente.
...
2. Cada mes, reemplazar `Lista de Precio MAYORISTA - Base de Datos.xlsx` conservando nombre y estructura.
3. Abrir o recargar el HTML desde hosting o servidor local. Lee el Excel automáticamente, sin usar la caché.
4. El cruce es por SKU: columna A (`Código`), columna L (neto) y columna M (final con IVA).
5. Revisar el estado de lectura de precios antes de imprimir o exportar a PDF.

Se eliminó el botón de actualización manual y el CSV. No se buscan archivos alternativos XLS o CSV.
